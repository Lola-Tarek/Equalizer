...
class Window(QtWidgets.QMainWindow, mainlayout.Ui_MainWindow):
    ...
    def __init__(self):
        ...
        self.actionZoom_In.triggered.connect(lambda: self.zoom(0.9, self.original_waveform))
        self.actionZoom_In.triggered.connect(lambda: self.zoom(0.9, self.modified_waveform))
        self.actionZoom_out.triggered.connect(lambda: self.zoom(1/0.9, self.original_waveform))
        self.actionZoom_out.triggered.connect(lambda: self.zoom(1/0.9, self.modified_waveform))
        ...
        self.action_Signal_beginning.triggered.connect(lambda: self.signal_beginning(self.original_waveform))
        self.action_Signal_beginning.triggered.connect(lambda: self.signal_beginning(self.modified_waveform))
        self.actionRight.triggered.connect(lambda: self.scroll(-10,-20,self.original_waveform))  ##scroll to left
        self.actionRight.triggered.connect(lambda: self.scroll(-10,-20,self.modified_waveform) ) ##scroll to left
        self.actionLeft.triggered.connect(lambda: self.scroll(20,10,self.original_waveform))  ##scroll to Right
        self.actionLeft.triggered.connect(lambda: self.scroll(20,10,self.modified_waveform))  ##scroll to Right
        self.actionSignal_End.triggered.connect(lambda: self.signal_end(self.original_waveform))
        self.actionSignal_End.triggered.connect(lambda: self.signal_end(self.modified_waveform))
   ...
   # play function and play as fast as possible
    ...
    def update (self):
        ...
        self.plot(self.modified_data)
        starting_x = self.original_waveform.getAxis("bottom").range
        self.modified_waveform.setXRange(starting_x[0] ,starting_x[1] ,padding=0.005)
        self.verticalLayout_7.addWidget(self.modified_waveform)
        self.spec_sliders()
        self.open_window()
        self.get_amplitude()
    ...
    # to signal beginning
    def signal_beginning(self, waveform):
        self.pause_signal()
        # get original xrange
        x_range = [0, 2000]
        waveform.setXRange(x_range[0], x_range[1],padding=0.005)

    # to signal end
    def signal_end(self, waveform):
        self.pause_signal()
        # set xrange to be
        
        x_end = len(self.original_data)
        waveform.setXRange(x_end - 2000, x_end,padding=0.005)


    def scroll(self,start_offset, end_offset, waveform):
            x_start = waveform.getAxis("bottom").range[0]
            x_end = waveform.getAxis("bottom").range[1]
            
            if(start_offset<0 and (x_start + start_offset < 0)):
                self.signal_beginning(waveform)
            elif((x_end + end_offset > len(self.original_data)) and end_offset >0):
                self.signal_end(waveform)
            else:
                waveform.setXRange(x_start + start_offset, x_end + end_offset,padding=0.005)
    ...
    def spectro_draw(self,colorcmap):
        ...
        #plt.axis(ymin=self.specSlider1.value() ,ymax=self.specSlider2.value() )
        plt.xlabel('Time(sec)')
